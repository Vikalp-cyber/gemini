  ...
  void queryTextAndImage({required String query, required File image}) {
    ...
    gemini.generateFromTextAndImages(query: query, image: image).then((value) {
      ...
      setState(() {
        loading = false;
        textImageChat
            .add({"role": "Gemini", "text": value.text, "image": image});
      });
      scrollToEnd();
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  leading: CircleAvatar(
                    child: Text(textImageChat[index]["role"]
                        .substring(0, 1)), //first letter of role as Avatar
                  ),
                  title: Text(textImageChat[index]["role"]), //username=>role
                  subtitle: Text(textImageChat[index]["text"]), //chat=>text
                  trailing: textImageChat[index]["image"] == ""
                      ? null
                      : GestureDetector(
                          onTap: () {
                            setState(() {
                              imagePreview = textImageChat[index]["image"];
                            });
                            _showImagePreview(context);
                          },
                          child: Image.file(
                            textImageChat[index]["image"],
                            width: 80,
                          ),
                        ),
                );
